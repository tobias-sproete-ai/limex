import GoldbachCircleMethodCenteredPrincipalNetExponentV18504
import GoldbachCircleMethodSupportSeparatedCentralMomentV18485

/-!
# Goldbach V1.8.505: hybrid-centered central moment

The corrected V1.8.501 three-channel energy is propagated through the actual
source block and central target sweep.  This supersedes the older budget only
at the principal channel: `Lambda - 1` remains intact until its local
discrepancy estimate is applied.
-/
...
namespace GoldbachCircleMethodHybridCenteredCentralMomentV18505
...
/-- Exact corrected source-side budget over the actual dyadic block. -/
noncomputable def hybridCenteredSourceBudget
    (B : ℕ) (rho _b : ℝ)
...
theorem hybridCenteredSourceBudget_nonneg
    (B : ℕ) (rho b : ℝ)
    (e : CharacterSlot ⌊((B : ℝ) ^ rho) ^ 2⌋₊) :
    0 ≤ hybridCenteredSourceBudget B rho b e := by
  unfold hybridCenteredSourceBudget
  apply Finset.sum_nonneg
  intro n _hn
  have hprincipalCoefficient : 0 ≤ centeredPrincipalCoefficientEnergy
      ⌊((B : ℝ) ^ rho) ^ 2⌋₊ n
      (logWeight ((B : ℝ) ^ rho) canonicalLogBump) := by
    unfold centeredPrincipalCoefficientEnergy
    positivity
  have hprincipalSource : 0 ≤ centeredPrincipalSourceEnergy
      ⌊((B : ℝ) ^ rho) ^ 2⌋₊ B n
      ((B : ℝ) / ((B : ℝ) ^ rho) ^ 4) := by
    unfold centeredPrincipalSourceEnergy
    positivity
  have hnonprincipalCoefficient : 0 ≤ nonprincipalCoefficientEnergy
      ⌊((B : ℝ) ^ rho) ^ 2⌋₊ n
      (logWeight ((B : ℝ) ^ rho) canonicalLogBump) := by
    unfold nonprincipalCoefficientEnergy
    positivity
  have hnonprincipalSource : 0 ≤ nonprincipalPrimitiveSourceEnergy
      ⌊((B : ℝ) ^ rho) ^ 2⌋₊ B n
      ((B : ℝ) / ((B : ℝ) ^ rho) ^ 4) := by
...
/-- The literal supported error energy is bounded by the corrected hybrid
budget. -/
theorem supportedAdjustedErrorEnergy_le_three_hybridCenteredSourceBudget
    (B : ℕ) (rho b : ℝ)
    (e : CharacterSlot ⌊((B : ℝ) ^ rho) ^ 2⌋₊)
    (hH : 1 ≤ (B : ℝ) / ((B : ℝ) ^ rho) ^ 4)
    (hb : 0 ≤ b) :
    supportedAdjustedErrorEnergy B rho b e ≤
      3 * hybridCenteredSourceBudget B rho b e := by
  unfold supportedAdjustedErrorEnergy hybridCenteredSourceBudget
  rw [Finset.mul_sum]
  ...
  exact adjustedCenteredError_sq_le_hybrid
    ⌊((B : ℝ) ^ rho) ^ 2⌋₊ B n
    ((B : ℝ) / ((B : ℝ) ^ rho) ^ 4) b
...
sweep. -/
theorem centralAdjustedCorrectionEnergy_le_hybridCentered
    (m : ℕ) (rho b : ℝ)
  ...
  have hsource :=
    supportedAdjustedErrorEnergy_le_three_hybridCenteredSourceBudget
  ...
  calc
    ...
    _ = 3 * hybridCenteredSourceBudget (8 * m) rho b e.val *
        (∑ i ∈ Finset.range (2 * m + 1),
          adjustedCorrectionPartnerEnergy
            (8 * m) (centralTargetNat m i) rho b e.val) := by
      rw [Finset.mul_sum]

/-- The central signed error aggregate inherits the corrected hybrid budget. -/
theorem central_adjusted_error_target_sum_abs_sq_le_hybridCentered
    (m : ℕ) (rho b : ℝ)
    (e : StructurallyAdmissibleActiveSlot
      ⌊(((8 * m : ℕ) : ℝ) ^ rho) ^ 2⌋₊)
    (hH : 1 ≤ ((8 * m : ℕ) : ℝ) /
  ...
  calc
    ...
    _ ≤ (2 * m + 1 : ℕ) *
        (3 * hybridCenteredSourceBudget (8 * m) rho b e.val *
          centralAdjustedPartnerEnergy m rho b e) := by
      exact mul_le_mul_of_nonneg_left
        (centralAdjustedCorrectionEnergy_le_hybridCentered
          m rho b e hH hb) (by positivity)

end GoldbachCircleMethodHybridCenteredCentralMomentV18505
