import GoldbachCircleMethodSupportedModelErrorConvolutionV18182
...
namespace GoldbachCircleMethodUncorrectedSupportedErrorConvolutionV18184

/-- The literal conductor-one principal model, with the original half-window
support and no exceptional-character correction. -/
noncomputable def supportedPrincipalModel (M N : ℕ) (R : ℝ)
    (G : ℝ → ℝ) : ℂ :=
  if hR : 1 < R then
...
/-- The literal V157 centered error on the original Lambda block and scale
`H=M/R^4`, with no `CharacterSlot` and no exceptional correction. -/
noncomputable def supportedUncorrectedError (M N : ℕ) (R : ℝ)
    (G : ℝ → ℝ) : ℂ :=
  if N ∈ Finset.Ioc (M / 2) M then
    centeredWindowError ⌊R^2⌋₊ N ((M : ℝ) / R^4)
      (blockCarrier M) (blockInput M) (logWeight R G)
  else 0

/-- The supported principal model has a radical-majorant constant fixed by
`G` before any scale exponent is chosen. -/
theorem supported_principalModel_radical_bound {G : ℝ → ℝ}
    (hc : HasCompactSupport G) (hd : ContDiff ℝ ∞ G)
    (hG : ∀ t : ℝ, 2 < t → G t = 0) :
    ∃ CB : ℝ, 0 ≤ CB ∧ ∀ (R : ℝ), 2 ≤ R →
      ∀ (M N : ℕ), ‖supportedPrincipalModel M N R G‖ ≤
        CB * radicalMajorant R N := by
  ...
  · rw [if_neg hn,norm_zero]
    exact mul_nonneg hC (radicalMajorant_nonneg hR1 N)

/-- Literal finite uncorrected `E*(a+B)` norm majorant. Both constants are
fixed by `G` before `rho`; the actual V157 mass estimate and all analytic
source inputs remain explicit hypotheses. -/
theorem eventual_supported_uncorrected_error_model_lambda_bound {G : ℝ → ℝ}
    (hc : HasCompactSupport G) (hd : ContDiff ℝ ∞ G)
    (hG : ∀ t : ℝ, 2 < t → G t = 0) :
    ∃ CE : ℝ, 0 ≤ CE ∧ ∃ CB : ℝ, 0 ≤ CB ∧
      ...
      ∃ M₀ : ℕ, ∀ {M m : ℕ} (eta : ℝ),
        ...
        (∀ n ∈ Finset.Ioc (M / 2) M,
          actualFluctuationMass ⌊((M : ℝ)^rho)^2⌋₊ n
            ((M : ℝ) / ((M : ℝ)^rho)^4) (blockCarrier M) (blockInput M) ≤ eta) →
        (∑ n ∈ Finset.Ioc (M / 2) M,
          ‖supportedUncorrectedError M n ((M : ℝ)^rho) G‖ *
            (‖blockInput M (m - n)‖ +
              ‖supportedPrincipalModel M (m - n) ((M : ℝ)^rho) G‖)) ≤
          CE * eta *
  ...
  obtain ⟨CE,hCE,herror⟩ := actual_centered_error_radical_bound hc hd hG
  ...
  refine ⟨M₀,?_⟩
  intro M m eta hM₀ hmlower hmupper heta hsource hmass
  have hs := hscale M hM₀
  have hR1 := (fixedRho_scale_admission hs.1 hrho hrho1).1
  have hLambda := actual_half_scale_lambda_power_bound D c CH hD hCH
  ...
  have hpoint (n : ℕ) (hn : n ∈ Finset.Ioc (M / 2) M) :
    ...
    have hnpos : 0 < n := by
      simp only [Finset.mem_Ioc] at hn
      omega
    have herr0 := herror ((M : ℝ)^rho) hs.2 n hnpos
      ((M : ℝ) / ((M : ℝ)^rho)^4) (blockCarrier M) (blockInput M)
    have hrad : 0 ≤ radicalMajorant ((M : ℝ)^rho) n :=
      radicalMajorant_nonneg hR1 n
    have herr : ‖supportedUncorrectedError M n ((M : ℝ)^rho) G‖ ≤
        (CE * eta) * radicalMajorant ((M : ℝ)^rho) n := by
      rw [supportedUncorrectedError,if_pos hn]
      calc
        _ ≤ CE * radicalMajorant ((M : ℝ)^rho) n *
            actualFluctuationMass ⌊((M : ℝ)^rho)^2⌋₊ n
              ((M : ℝ) / ((M : ℝ)^rho)^4) (blockCarrier M) (blockInput M) := herr0
...
end GoldbachCircleMethodUncorrectedSupportedErrorConvolutionV18184
