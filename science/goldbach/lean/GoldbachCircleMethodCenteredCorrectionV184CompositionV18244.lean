import GoldbachCircleMethodCenteredCorrectionLiteralNormAdapterV18243

/-!
# Goldbach V1.8.244: centered-correction V1.8.184 composition

The literal norm adapter is composed with the existing finite
uncorrected-error theorem.  The result gives an explicit pointwise bound for
the exact centered-error correction while keeping every external analytic
input visible: prefix estimates, arithmetic correlation, and uniform actual
character-family fluctuation mass.
-/

open scoped Classical BigOperators ArithmeticFunction

set_option autoImplicit false

namespace GoldbachCircleMethodCenteredCorrectionV184CompositionV18244

open GoldbachCircleMethodFiniteCompanionBindingV18118
open GoldbachCircleMethodLogCutoffPresieveBindingV18120
open GoldbachCircleMethodBumpDecayRadicalEnvelopeV18155
open GoldbachCircleMethodActualCharacterRadicalErrorV18157
open GoldbachCircleMethodRadicalCorrelationTransferV18161
open GoldbachCircleMethodBadPrimeProductComparisonV18165
open GoldbachCircleMethodMertensUniformConstantsV18168
open GoldbachCircleMethodBadPrimeUniformFrequencyV18169
open GoldbachCircleMethodGlobalSieveFiniteFactorV18173
open GoldbachCircleMethodMertensProductFromHarmonicV18174
open GoldbachCircleMethodDisjointPrimeFactorCompositionV18171
open GoldbachCircleMethodActualHalfScaleCorrelationV18177
open GoldbachCircleMethodDirectLambdaRadicalDominationV18178
open GoldbachCircleMethodFixedRhoPolynomialLossV18180
open GoldbachCircleMethodEventualScaleActualErrorLambdaV18181
open GoldbachCircleMethodUncorrectedSupportedErrorConvolutionV18184
open GoldbachCircleMethodCanonicalBumpPrincipalFloorV18220
open GoldbachCircleMethodCanonicalBumpResidualMomentsV18222
open GoldbachCircleMethodActualResidualExactDecompositionV18225
open GoldbachCircleMethodCenteredCorrectionLiteralNormAdapterV18243

/-- Exact pointwise bound for the literal centered-error correction under the
unchanged analytic premises of V1.8.184.  No source estimate or fluctuation
mass bound is constructed by this theorem. -/
theorem eventual_canonicalCenteredErrorCorrectionAt_bound :
    ∃ CE : ℝ, 0 ≤ CE ∧ ∃ CB : ℝ, 0 ≤ CB ∧
      ∀ (D c CH : ℝ) (_hD : 0 ≤ D) (_hCH : 0 ≤ CH)
        (_hweighted : ∀ (X : ℕ), 2 ≤ X → ∀ t : ℝ, 2 ≤ t → t ≤ (X : ℝ) →
          |prefixLogMoment X t - Real.log t| ≤ D)
        (_hharmonic : ∀ (X : ℕ), 2 ≤ X → ∀ t : ℝ, 2 ≤ t → t ≤ (X : ℝ) →
          |prefixHarmonicMoment X t - Real.log (Real.log t) - c| ≤
            D / Real.log t)
        (rho : ℝ) (_hrho : 0 < rho) (_hrho1 : rho ≤ 1),
      ∃ B₀ : ℕ, ∀ {B N : ℕ} (eta : ℝ),
        B₀ ≤ B → 5 * B ≤ 4 * N → 4 * N ≤ 7 * B → 0 ≤ eta →
        (∀ ξ η' : ℝ,
          arithmeticCorrelation ((B : ℝ)^rho) ξ η'
              (Finset.Ioc (B / 2) B) N ≤
            CH * ((B : ℝ) / 2) * finiteSieveProduct N (B / 2) *
              goodBadProduct ((B : ℝ)^rho) ξ η' N (B / 2)) →
        (∀ n ∈ blockCarrier B,
          actualFluctuationMass ⌊((B : ℝ)^rho)^2⌋₊ n
            ((B : ℝ) / ((B : ℝ)^rho)^4)
            (blockCarrier B) (blockInput B) ≤ eta) →
        ‖canonicalCenteredErrorCorrectionAt B rho (N : ℤ)‖ ≤
          CE * eta *
            ((128 * CH * (mertensProductConstant D c)^2 *
                (oneFrequencyConstant D c)^2 *
                (∫ ξ : ℝ, secondMomentDecay ξ)^2 / tenthDecayMass) *
              (B : ℝ) * arithmeticFactor N / rho^3 +
             CB * (128 * CH * (mertensProductConstant D c)^2 *
                (oneFrequencyConstant D c)^2 *
                (∫ ξ : ℝ, secondMomentDecay ξ)^2) *
              (B : ℝ) * arithmeticFactor N / rho^2) := by
  obtain ⟨CE, hCE, CB, hCB, hCore⟩ :=
    eventual_supported_uncorrected_error_model_lambda_bound
      canonicalLogBump_hasCompactSupport canonicalLogBump_contDiff
      (fun _ hx => canonicalLogBump_zero_above_two hx)
  refine ⟨CE, hCE, CB, hCB, ?_⟩
  intro D c CH hD hCH hweighted hharmonic rho hrho hrho1
  obtain ⟨B₀, hBound⟩ :=
    hCore D c CH hD hCH hweighted hharmonic rho hrho hrho1
  refine ⟨B₀, ?_⟩
  intro B N eta hB₀ hLower hUpper heta hsource hmass
  have hMajor := hBound eta hB₀ hLower hUpper heta hsource (by
    simpa only [blockCarrier] using hmass)
  have hAdapter := norm_canonicalCenteredErrorCorrectionAt_le_majorant
    B N rho (by omega)
  exact hAdapter.trans (by
    simpa only [centeredCorrectionMajorant, blockCarrier] using hMajor)

end GoldbachCircleMethodCenteredCorrectionV184CompositionV18244
